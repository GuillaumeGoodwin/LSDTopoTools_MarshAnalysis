...
def MarshID(Input_dir =  "/LSDTopoTools/LSDTopoTools_MarshPlatform/Example_data/",
    ...
    for site in Sites:
        ...
        # We need to improve the treatment of slope by removing the borders.
        ...
        Bins, Hist = Distribution (Slope, Nodata_value)
        ax1.plot( Bins, Hist, '-r', linewidth = 2.0)
        ax1.set_xlim (0,1.5)

        
        Max = np.where (Hist >= 0.9 * max(Hist))
        
        print Hist
        print max (Hist), Max
        
        Slope [Slope > 1] = Nodata_value
        
        Bins, Hist = Distribution (Slope, Nodata_value)
        ax1.plot( Bins, Hist, '-b', linewidth = 2.0)
        
        print Hist
        
        plt.savefig ("/home/s1563094/Datastore/Software/LSDTopoTools/LSDTopoTools_MarshAnalysis/Output/"+'Slope_pdf.png')
